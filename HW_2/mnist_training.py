...
class Network:
    ...
    #REDO with number of epochs
    @staticmethod
    def get_decay_rate(learning_rate, final_learning_rate, e):
        decay_rate = (final_learning_rate/learning_rate)**(1/(e-1))
        return decay_rate
    ...
    def construct(self, args):
        with self.session.graph.as_default():
            ...
            if not args.learning_rate_final:
                learning_rate = args.learning_rate
            else:
                decay_rate = self.get_decay_rate(args.learning_rate, args.learning_rate_final, args.epochs)
                learning_rate = tf.train.exponential_decay(learning_rate=args.learning_rate,
                                                               global_step=global_step,
                                                               decay_steps=batches_per_epoch, decay_rate=decay_rate,
                                                               staircase=True)
    ...
    def evaluate(self, dataset, images, labels):
        acc, summs = self.session.run([self.accuracy, self.summaries[dataset]], {self.images: images, self.labels: labels})
        return acc
